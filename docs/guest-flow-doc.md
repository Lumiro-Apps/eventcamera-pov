...
```
https://guest.poveventcam.app/e/{event_slug}
```
...
1. The app shows the event name and optional branding (e.g., "Upload to Rohit & Jyoti's Wedding").
2. If the event has a PIN enabled, the guest is prompted to enter a 4-digit PIN.
3. Optionally, the guest can enter a display name (e.g., "Photos by Amit").
4. The client calls `POST /api/join` with the event slug and optional PIN.

**What the server does:**

1. Validates the event slug exists and the event status is `active`.
2. If the event has `expires_at` in the past, rejects with `EVENT_CLOSED`.
3. If PIN is enabled, validates the PIN against the stored hash.
4. Performs an **atomic guest count check and reservation**:
...
- **Optional display name** — editable, saved via a PATCH call.
...
- **Cannot upload after event closes.** The `create-upload` endpoint checks `event.status` and `event.expires_at`.
