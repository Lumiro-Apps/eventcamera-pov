...
The backend is a Node.js/Express API server that handles metadata operations, session management, quota enforcement, and signed URL generation. It never handles file binary data — all file uploads and downloads flow directly between the client and Supabase Storage.
...
- **Deployment:** Railway (single instance for MVP, horizontally scalable later)
...
- **Role:** File storage for photos and thumbnails
- **Backend:** S3-compatible, managed by Supabase
- **Buckets:** Two private buckets — `originals` and `thumbs`
...
- **Provides:** JWT-based auth with email/password, magic link, Google OAuth
...
> [!NOTE]
> Supabase CDN has limitations: no custom cache rules, no edge locations in India (closest: Singapore), no cache purge API. For MVP this is acceptable; Stage 2 should use Cloudflare CDN in front of R2/B2.
...
> [!IMPORTANT]
> `compression_mode` can only be changed **before** the event starts (`starts_at`). Once `starts_at` is reached, the setting is locked.
...
All organizer endpoints require a valid Supabase Auth JWT.
...
Request
  ...
  → Route handler
    → Auth middleware (cookie-based for guest, JWT-based for organizer)
...
This is the central pattern of the entire system. The API server generates time-limited, scoped URLs that allow the client to interact with Supabase Storage directly.
...
const { data, error } = await supabase.storage
  .from('originals')
  .createSignedUploadUrl(`${eventId}/${mediaId}.${ext}`);
```

Properties:
- **Scoped to a single file path.** Cannot be reused for another file.
- **Expires in 2 hours** (Supabase default).
...
const { data, error } = await supabase.storage
  .from('thumbs')
  .createSignedUrl(`${eventId}/${mediaId}.jpg`, 3600); // 1 hour
```

Properties:
- **Scoped to a single file.**
- **Configurable expiry.** 1 hour for thumbnails, 15 minutes for original downloads.
- **Read-only.** Cannot be used to write or delete.

### 6.3 Why Signed URLs (Not RLS)

Supabase supports RLS policies on storage buckets, but signed URLs are a better fit here because:
...
**Logic:**
1. Attempt to fetch metadata for the thumb path from Supabase Storage.
...
3. Stream originals from Supabase Storage.
...
Optionally, a second pass archives events that have been closed for more than a configurable grace period (e.g., 30 days), and a third pass purges storage for events that have been archived for more than the retention period.
...
Supabase free tier: 1 GB storage. Supabase Pro: 100 GB included, then $0.021/GB.

At 10 events/month, you'll need Supabase Pro ($25/month) and may incur no overage. At 50 events/month, expect ~$6-8/month in storage overage.
...
| API backend          | Railway        | $5-7/month    |
| Database + Storage   | Supabase Pro   | $25/month     |
| Cache + Rate Limit   | In-memory      | $0            |
| Background Jobs      | Same process   | $0            |
| **Total**            |                | **~$50-52/month** |

Single Railway instance. Background jobs run in the same Node.js process. In-memory cache. No Redis.
...
| API backend          | Railway (2-3 instances) | $15-20/month |
| Database + Storage   | Supabase Pro   | $25 + overage |
| Redis (cache + rate limit) | Upstash   | $10/month     |
| Background Jobs      | Separate Railway worker | $5-7/month |
...
| API backend          | Railway / ECS / Fly.io | Auto-scaling containers |
| Database             | Supabase Pro or dedicated Postgres | Connection pooling critical |
| Storage              | Supabase or raw S3 + CloudFront | Evaluate cost |
| Job Queue            | BullMQ + Redis | Dedicated queue |
| CDN                  | Cloudflare Pro | Caching layer |

At this stage, evaluate whether Supabase Storage pricing still makes sense vs. raw S3 + CloudFront. Abstract storage behind an interface from day one to make this migration painless.
...
Both storage buckets are private. No file is accessible without a signed URL generated by the API server. There are no public URLs in the system.

### 11.2 Signed URLs Are Scoped

- Upload URLs: write-only, single path, 2-hour expiry.
- Download URLs: read-only, single path, 1-hour expiry (thumbnails) or 15-minute expiry (originals).
...
- All organizer endpoints verify the JWT and check that the organizer owns (or is a collaborator on) the requested event.
